...
<script>
	jQuery(document).ready(function() {
		var $status = jQuery('.pageinfo');
		var $slickElement = jQuery('.customtestimonial');
...
if($query->have_posts()){
	...
	while($query->have_posts()){
		...
		$image_url = (wp_get_attachment_url($image_id))?wp_get_attachment_url($image_id):site_url()."/wp-content/uploads/2018/08/testimonil-2.png";
		$content = get_the_content();
		$output .= "<div class='col-md-3'>";
		$output .= "<div class='grid_item thumbnail'>";

		$output .= '<div style="visibility:hidden;" id="testimonial_id">'.$count.'</div>';	
		...
		if($title == 'true' || $description == 'true'){
			...
			if($link_on_featuredimage == 'true'){
				...
				$output .= "<div class='carousel-image'><img src='".$image_url."' alt='' /></div>";
			}
		...
		if($featuredimage == 'true'){
			...
			if($title == 'true'){
				...
				if($title_position == 'after_description'){
					$output .= "<div class='item_title'>";
					if($link_on_title == 'true'){
						$output .= "<div class='title-carousel'><a href='".get_the_permalink()."'>".get_the_title()."</a></div>";
					}else{
						$output .= get_the_title();
					}
					$output .= "</div>";
			...
			if($km_country_show_hide == 'true'){
				$output .= "<div class='designation-carousel'><div class='designaiton'>";
			...
			if($km_city_show_hide == 'true'){
				$output .= "<div class='designation-carousel2'><div class='conpanyname'>";
		...
		// $count_posts = wp_count_posts('km_testimonial');
		// $total_posts = $count_posts->publish;
		// $output .=  '<span class="testimonial_post">'.$count.'</span>/'.$total_posts;
